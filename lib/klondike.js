...
const KlondikeField = () => {
    ...
    return inherit(ClickArea(), {
        ...
        findClick(engine, pos) {
            ...
            if (this.deck.intersect(assetLoader, deckPos, pos)) {
                return {
                    location: "deck", clickable: true,
                    offset: this.deck.intersectSubindexOffset(assetLoader, deckPos, pos).offset
                };
            }

            const dropPos = { x: width + width / 2, y: 0 };
            if (this.drop.intersect(assetLoader, dropPos, pos)) {
                return {
                    location: "drop",
                    offset: this.drop.intersectSubindexOffset(assetLoader, dropPos, pos).offset
                };
            }
            ...
            for (let i = 0; i < this.foundations.length; i++) {
                ...
                if (foundation.intersect(assetLoader, stackPos, pos)) {
                    return {
                        location: "foundation", index: i,
                        offset: foundation.intersectSubindexOffset(assetLoader, stackPos, pos).offset
                    };
                }
            ...
            for (let i = 0; i < this.stacks.length; i++) {
                ...
                if (stack.intersect(assetLoader, stackPos, pos)) {
                    return {
                        location: "stack", index: i,
                        subindex: stack.intersectSubindex(assetLoader, stackPos, pos),
                        offset: stack.intersectSubindexOffset(assetLoader, stackPos, pos).offset
                    };
                }
        ...
        draw(canvas, assetLoader) {
            ...
            if (this.dragged.cards.length > 0) {
                ...
                canvas.ctx.translate(
                    this.dragPos.x - this.dragStartTarget.offset.x,
                    this.dragPos.y - this.dragStartTarget.offset.y
                );
