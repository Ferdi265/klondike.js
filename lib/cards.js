    ...
    return inherit(null, {
        ...
        bounds(assetLoader) {
            ...
            if (this.cards.length > 1) {
                if (this.xoffset < 0) {
                    rect.left += this.xoffset * width * (this.cards.length - 1);
                } else {
                    rect.right += this.xoffset * width * (this.cards.length - 1);
                }
                if (this.yoffset < 0) {
                    rect.top += this.yoffset * height * (this.cards.length - 1);
                } else {
                    rect.bottom += this.yoffset * height * (this.cards.length - 1);
                }
            }
        ...
        intersect(assetLoader, stackPos, pos) {
        ...
        intersectSubindex(assetLoader, stackPos, pos) {
            return this.intersectSubindexOffset(assetLoader, stackPos, pos).index;
        },

        intersectSubindexOffset(assetLoader, stackPos, pos) {
            const cardNone = CardNone();
            const texture = assetLoader.get(cardNone.texture());
            const width = texture.width;
            const height = texture.height;

            let xoffset = width * this.xoffset * this.cards.length;
            let yoffset = height * this.yoffset * this.cards.length;
            for (let i = this.cards.length - 1; i >= 0; i--) {
                xoffset -= width * this.xoffset;
                yoffset -= height * this.yoffset;
                ...
                if (rectContains({
                    ...
                    return {
                        index: i,
                        offset: posOffset({
                            x: stackPos.x + xoffset,
                            y: stackPos.y + yoffset
                        }, pos)
                    };
                }
            }
